/**
彩云天气 v2.0 自用 - 自动触发版本 (企业微信推送版)
@author: lizhengbo
更新地址：https://raw.githubusercontent.com/Lizhengbo0304/qx_boxjs/refs/heads/main/caiyun_qywx.js
 *
功能：
√ 监控高德API自动获取位置
√ 异常天气预警
...
async function processWeatherData(weatherData, address) {
  try {
    ...
    // 发送实时天气通知
    await sendRealtimeWeatherNotification(weatherData.result, address);
...
async function sendRealtimeWeatherNotification(data, address) {
  // 添加数据验证，防止undefined错误
  ...
  const title = `[彩云天气] ${address}`;
  const content = `${mapSkycon(realtime.skycon)[0]} ${realtime.temperature} ℃  🌤 空气质量 ${realtime.air_quality.description.chn}\n\n` + 
                  `🔱 ${keypoint}\n` + 
...
async function sendQYWX(title, content) {
    ...
    const url = `https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=${qywxKey}`;
    const body = {
        msgtype: "markdown",
        markdown: {
    ...
    try {
        ...
        if (res.errcode === 0) {
            ...
            $.log(`❌ 企业微信推送失败: ${res.errmsg}`);
        }
    } catch (e) {
        $.log(`❌ 企业微信推送异常: ${e.message}`);
    }
}
